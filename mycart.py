...
def checkout(username,password):
    """Buy all products from cart checkout"""
    ...
    if user_name is not None and username == user_name[0] and password == user_name[1]:
        ...
        if check_cart:
            ...
            id = random.randint(1, 100000)
            conn.execute("INSERT INTO BILLS VALUES (?,?,?,?,?)", (id,user_name,amount, discount, final_amount ))
            conn.commit()
        else:
...
@cli.command()
@click.password_option('--adminpassword',confirmation_prompt=False,required=True)
def view_users_cart(adminpassword):
    """View all the products added to the cart by users"""
    conn = create_db()
    cur = conn.cursor()
    cur.execute("SELECT password from ADMIN")
    password = cur.fetchone()
    if password and adminpassword == password[0]:
         print(pd.read_sql_query("SELECT * from MYCART",conn))
    else:
        click.echo('Incorrect password')   
...
@cli.command()
@click.password_option('--adminpassword',confirmation_prompt=False,required=True)
def view_all_bills(adminpassword):
    """View all bills generated by all the users"""
    conn = create_db()
    cur = conn.cursor()
    cur.execute("SELECT password from ADMIN")
    password = cur.fetchone()
    if password and adminpassword == password[0]:
         print(pd.read_sql_query("SELECT * from BILLS",conn))
    else:
        click.echo('Incorrect password') 
